	...
	go func() {
		for {
			...
			case fevent, ok := <-g.Watcher.Events:
				...
				gevent := NotifyToGorphEvent(g, &fevent)
				switch gevent.Op {
				case FolderAdded:
					g.addFolder(gevent)
				case FolderRemoved:
				...
				case FolderRenamed:
					//	rename carries the old filename
					//	the new filename comes in a new CREATE event
					//	which will covered in the next go around
					g.removeFolder(gevent)
				}
				g.RLock()
				if !g.isClosed {
...
func New(globPattern string) (Gorph, error) {
	root, pattern := doublestar.SplitPattern(globPattern)
	backingFs := os.DirFS(root)
	return NewWithFs(root, pattern, backingFs)
}
